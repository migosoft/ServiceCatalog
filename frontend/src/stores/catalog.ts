...
export const useCatalogStore = defineStore('catalog', () => {
  ...
  async function createNode(data: { type: string; name: string; description?: string; operatingSystem?: string; owner?: string; address?: string; dbType?: string }) {
    const node = await catalogApi.createNode(data)
    nodes.value.push(node)
    return node
  }

  async function updateNode(id: string, data: { name: string; description?: string; operatingSystem?: string; owner?: string; address?: string; dbType?: string }) {
